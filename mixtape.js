...
let generateLink = document.querySelector('[data-generate="link"]');
generateLink.addEventListener("click", function () {
  linkText = generateLink.querySelector('[data-generate="text"]');
  linkSuccessText = generateLink.getAttribute("data-success");
  ...
  setTimeout(() => {
    linkText.textContent = "Generate share link";
  }, 2000);
});
...
function initSoundtracks(soundtracksMap) {
  ...
  if (entries.length === 0) {
    ...
    const [firstId, firstSrc] = entries[0];
    const firstAudioPlayer = soundTemplate.querySelector(".audio-player");
    if (firstAudioPlayer) {
      firstAudioPlayer.setAttribute("data-audio-id", firstId);
      firstAudioPlayer.setAttribute("data-audio-src", firstSrc);
    }

    // Clone and update for the rest
